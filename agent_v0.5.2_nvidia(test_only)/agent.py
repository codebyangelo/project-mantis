...
class VerdictInfo(BaseModel):
    ...
    exact_telemetry_quote: str = Field(description="CRITICAL: MUST be a verbatim, character-for-character substring copy-pasted directly from the raw JSON telemetry. Do not summarize or combine fields. Example: '\"inode\": \"12345\"'. If you cannot extract a perfect raw substring, output 'NONE'.")
...
class MantisAgent:
    ...
    def __init__(self):
        ...
        elif self.provider in ["openai", "local", "groq", "cerebras", "nvidia"]:
            try:
                import openai
            except ImportError:
