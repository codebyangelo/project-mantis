...
class ExecutiveSynthesis(BaseModel):
    who_attributed: str = Field(description="Who is the threat actor? (Use OSINT/Web search on the C2 IPs or TTPs to attribute it to an APT group if possible)")
...
class MantisAgent:
    ...
    """
    def __init__(self):
        ExecutionLogger.log("AGENT", "Initializing Gemini MantisAgent with Exhaustive Search & Scoring.")
        # Check if they are using an API key or standard GCP Auth
        if os.environ.get("VERTEX_API_KEY"):
            # Vertex API Keys (from Google Cloud Console) are mutually exclusive with Project/Location params
        ...
        You are the Universal Forensic Engine (v0.2.7). 
    ...
    def synthesize_investigation(self, isolated_threats: list) -> ExecutiveSynthesis:
        ...
        prompt = "Based on the following isolated evidence, reconstruct the timeline and write an Incident Response narrative. Answer: Who is the threat actor? (Perform an OSINT Web Search on the IPs or TTPs to find out). What key projects were accessed? What was stolen? Where was it transferred to? How was it stolen? When did this occur? (Use the macb_timestamps to build a timeline). If the evidence does not support an answer, state 'Unknown based on evidence'.\n\nISOLATED THREATS:\n"
        for idx, threat in enumerate(isolated_threats):
            ...
            # Explicitly include the evidence if available so the synthesizer sees timestamps
            if 'evidence' in threat:
                prompt += f"Evidence: {json.dumps(threat['evidence'])}\n"
        
        config = types.GenerateContentConfig(
            system_instruction="You are a Lead Forensic Investigator (v0.2.7). Synthesize triage data into a cohesive incident report. You have access to Google Search. Use it to lookup IP addresses or TTPs to attribute the attack to an APT group.",
            temperature=0.2,
            response_mime_type="application/json",
