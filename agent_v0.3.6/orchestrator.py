    ...
    for ent in entities:
        ...
        try:
            ...
            if quote != "NONE" and quote not in raw_ev_str and quote_escaped not in raw_ev_str:
                ExecutionLogger.log("ORCHESTRATOR", f"CITATION TRAP ACTIVATED: LLM hallucinated evidence '{quote}'. Forcing BENIGN.", "ERROR")
                eval_result.verdict.classification = "BENIGN"
                eval_result.verdict.confidence_reasoning = f"FORCED BENIGN due to hallucinated citation."
            ...
            if eval_result.verdict.classification.upper() in ["MALICIOUS", "SUSPICIOUS"]:
                for ban in inference_bans:
                    if re.search(ban, reasoning_lower):
                        ExecutionLogger.log("ORCHESTRATOR", f"CLAIM GUARD ACTIVATED: Semantic inference detected ({ban}). Forcing BENIGN.", "ERROR")
                        eval_result.verdict.classification = "BENIGN"
                        eval_result.verdict.confidence_reasoning = f"FORCED BENIGN due to Semantic Inference Violation ({ban}). Original: {eval_result.verdict.confidence_reasoning}"
                        break
