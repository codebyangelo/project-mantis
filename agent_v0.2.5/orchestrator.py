...
def generate_mitre_report(results: list, agent_system=None):
    ...
    report_content = f"""# Project Find Evil (v0.2.5) - Autonomous DFIR Report
...
def run_fsm_loop(chat_session, agent_system=None):
    ...
    for ent in entities:
        ...
        investigation_results.append({"pid": ent["label"], "result": eval_result})  # Map 'label' to 'pid' for backwards compatibility in reports
...
def main():
    print(f"\033[96m[ PROJECT FIND EVIL - V0.2.5 (EXHAUSTIVE MODE) ]\033[0m")
    print("------------------------------------------------------------")
