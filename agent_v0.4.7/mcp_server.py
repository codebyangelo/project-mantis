...
def parse_registry_key(inode: str, disk_image_path: str, plugin: str) -> str:
    ExecutionLogger.log("MCP_SERVER", f"Starting Registry Parse (Inode: {inode}, Plugin: {plugin})")
    ...
    try:
        # Extract hive to temp file
        temp_hive = f"/tmp/hive_{inode.split('-')[0]}"
        img_type = "ewf" if disk_image_path.lower().endswith(".e01") else "raw"
        icat_cmd = ["icat", "-i", img_type, disk_image_path, inode.split('-')[0]]
        with open(temp_hive, "wb") as f:
            subprocess.run(icat_cmd, stdout=f, timeout=30)
            
        rip_cmd = ["rip.pl", "-r", temp_hive, "-p", plugin]
        ExecutionLogger.log("MCP_SERVER", f"Executing RegRipper: {' '.join(rip_cmd)}")
        
        proc = subprocess.run(rip_cmd, capture_output=True, text=True, timeout=120)
        
        if os.path.exists(temp_hive):
            os.remove(temp_hive)
            
        if proc.returncode != 0 and not proc.stdout:
            ExecutionLogger.log("MCP_SERVER", f"RegRipper failed: {proc.stderr.strip()}", "ERROR")
            return f"[!] RegRipper failed: {proc.stderr.strip()}"
            
        return proc.stdout[:8000] + "\n[!] OUTPUT TRUNCATED" if len(proc.stdout) > 8000 else proc.stdout
    except Exception as e:
        ExecutionLogger.log("MCP_SERVER", f"Pipeline error: {e}", "ERROR")
...
def search_disk_timeline(keyword: str) -> dict:
    """
    Searches the bodyfile and MFT caches for a specific keyword (e.g., PID or Process Name).
    """
    ExecutionLogger.log("MCP_SERVER", f"Executing search_disk_timeline(keyword='{keyword}')")
    results = []
    
    if os.path.exists(CACHE_DIR):
        for f in os.listdir(CACHE_DIR):
            if f.startswith("bodyfile_") or f.startswith("mft_"):
                    ...
                    with open(path, "r", encoding="utf-8", errors="ignore") as file:
                        for line in file:
                            if keyword.lower() in line.lower():
                                ...
                                results.append({"line": line.strip(), "inode": inode, "source_file": f})
                except Exception as e:
                    ExecutionLogger.log("MCP_SERVER", f"Error reading {f}: {e}", "ERROR")
    
    return {
        "status": "success",
        "results": results[:200]  # Limit to prevent context bloat
    }
