...
from mcp_server import (
    ...
    carve_memory_strings, read_dfir_playbook, run_with_timer, search_disk_timeline
)
...
def run_fsm_loop(chat_session, agent_system=None):
    ...
    for ent in entities:
        ...
        if ent_type in ("file", "registry_hive"):
            tag = ent.get("evidence", {}).get("lnk_baseline_tag", "")
            if ent.get("score", 0) < 99 and tag != "PATH_BASELINE_UNKNOWN":
                if ent.get("score", 0) < 100: # Only skip if < 99 and not 100
                    ExecutionLogger.log("ORCHESTRATOR", f"Disk entity '{ent['label']}' scored {ent.get('score')} (Not Critical). Cleared deterministically. Skipping LLM.")
                    continue
        ...
        # Merge LNK baseline tag if it exists
        lnk_tag = obfuscated_evidence.get("lnk_baseline_tag")
        if lnk_tag:
            if not baseline_tags: baseline_tags = []
            if lnk_tag not in baseline_tags:
                baseline_tags.append(lnk_tag)
        ...
        if ent_type == "process" and "malfind" in ent.get('evidence', {}) and ent['evidence']["malfind"]:
            try:
                ...
                allocs = [
                    ...
                    for a in ent.get('evidence', {}).get('malfind', [])
                ]
        ...
        if getattr(eval_result, "request_disk_search", ""):
            ...
            cmd = ["bash", "-c", f"grep -i '{ds_kw}' {CACHE_DIR}/bodyfile_*.txt | head -n 50"]
        ...
        if getattr(eval_result, "request_deep_carve", False):
            ...
            if ent_type == "process":
                ...
                # Deep carve is now deterministic in SMPT. No dynamic carve needed.
                pass
                ...
                try:
                    ...
                    if def_result.verdict.upper() == "OVERRULED_BENIGN":
                        if not def_result.citations:
                            ...
                            unescaped_ev = codecs.decode(json.dumps(obfuscated_evidence), 'unicode_escape')
            ...
            if ent_type == "process" and agent_system:
                ...
                if pid_match:
                    ...
                    if pivot_results and pivot_results.get("results"):
                        ...
                        # Extract content for the first result
                        content_sample = "No content extracted."
                        try:
                            first_res = pivot_results['results'][0]
                            inode = first_res.get("inode", "").split('-')[0]
                            source_file = first_res.get("source_file", "")
                            if inode and source_file:
                                disk_image_name = source_file.replace("bodyfile_", "").replace(".txt", "")
                                    ...
                                    with open(ctx_path) as ctxf:
                                        for p in json.load(ctxf).get("Evidence_Files", {}).get("Disk_Images", []):
                                            if disk_image_name in p:
                                                ...
                                                break
                                if real_disk_path:
                                    # Copy get_partition_offset logic or just run mmls directly here
                                    img_type = "ewf" if real_disk_path.lower().endswith(".e01") else "raw"
                                    proc = subprocess.run(["mmls", "-i", img_type, real_disk_path], capture_output=True, text=True)
                                    offset = ""
                                    ...
                                    icat_cmd.extend([real_disk_path, inode])
                                    icat_proc = subprocess.run(icat_cmd, capture_output=True)
                                    if icat_proc.returncode == 0:
                                        content_sample = icat_proc.stdout[:4096].decode('utf-8', errors='ignore')
                        except Exception as e:
                            content_sample = f"Extraction failed: {e}"
                        ...
                        EXTRACTED FILE CONTENT (FIRST 4KB):
                        {content_sample}
