...
from mcp_server import (
    ...
    extract_and_carve_hive, carve_memory_strings, read_dfir_playbook, run_with_timer
)
    ...
    for ent in entities:
        ...
        try:
            ...
            if quote != "NONE":
                ...
                norm_raw = re.sub(r'\s+', '', raw_ev_str)
                # also allow unescaped version match
                norm_raw_un = re.sub(r'\s+', '', json.dumps(obfuscated_evidence, ensure_ascii=False).replace('\\n', '').replace('\\t', ''))
                
                if norm_quote not in norm_raw and norm_quote not in norm_raw_un:
                    ExecutionLogger.log("ORCHESTRATOR", f"CITATION TRAP WARNING: LLM citation '{quote}' not exact match. Permitting anyway.", "WARN")
            ...
            if agent_system:
                ...
                try:
                    ...
                    if not def_result.citations and def_result.verdict.upper() == "OVERRULED_BENIGN":
                        ...
                        norm_raw_art = re.sub(r'\s+', '', raw_artifact_str)
                        norm_raw_un = re.sub(r'\s+', '', json.dumps(obfuscated_evidence, ensure_ascii=False).replace('\\n', '').replace('\\t', ''))
        ...
        if eval_result.verdict.classification.upper() == "MALICIOUS":
            ...
            if ent_type == "process" and agent_system:
                ...
                if pid_match:
                    ...
                    pivot_results = query_mcp("search_disk_timeline", {"keyword": f"{pid}"})
