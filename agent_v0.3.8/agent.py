...
class DefenseEvaluation(BaseModel):
    verdict: str = Field(description="OVERRULED_BENIGN or UPHELD_MALICIOUS")
...
class MantisAgent:
    ...
    def synthesize_investigation(self, isolated_threats: list) -> ExecutiveSynthesis:
        ...
        config = types.GenerateContentConfig(
            system_instruction="You are a Lead Forensic Investigator (v0.3.7). Synthesize triage data into a cohesive incident report. You have access to Google Search.",
            temperature=0.2,
            response_mime_type="application/json",
