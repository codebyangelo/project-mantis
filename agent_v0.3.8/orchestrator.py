...
from agent import MantisAgent, MantisEvaluation, DefenseEvaluation
...
def generate_mitre_report(results: list, agent_system=None):
    ...
    report_content = f"""# Project Mantis (v0.3.7) - Autonomous DFIR Report
...
def run_fsm_loop(chat_session, agent_system=None):
    ...
    for ent in entities:
        ...
        {json.dumps(obfuscated_evidence, indent=2)[:3000]}
        """
        ...
        try:
            ...
            quote_escaped = json.dumps(quote)[1:-1] if quote else "NONE"
            if quote != "NONE" and quote not in raw_ev_str and quote_escaped not in raw_ev_str:
                ExecutionLogger.log("ORCHESTRATOR", f"CITATION TRAP ACTIVATED: LLM hallucinated evidence '{quote}'. Forcing SUSPICIOUS.", "ERROR")
                eval_result.verdict.classification = "SUSPICIOUS"
                eval_result.verdict.confidence_reasoning = f"FORCED SUSPICIOUS due to hallucinated citation."

            # ClaimGuard Check
            inference_bans = [r'\bintent\b', r'\blikely\b', r'\bsuggests\b', r'\bimplies\b', r'\bindicates\b', r'\bmeaning of\b', r'\bresignation\b', r'\bsensitive\b', r'\bstaging\b', r'\bmotive\b']
            reasoning_lower = eval_result.verdict.confidence_reasoning.lower()
            if eval_result.verdict.classification.upper() in ["MALICIOUS", "SUSPICIOUS"]:
                for ban in inference_bans:
                    if re.search(ban, reasoning_lower):
                        ExecutionLogger.log("ORCHESTRATOR", f"CLAIM GUARD ACTIVATED: Semantic inference detected ({ban}). Forcing SUSPICIOUS for manual review.", "ERROR")
                        eval_result.verdict.classification = "SUSPICIOUS"
                        eval_result.verdict.confidence_reasoning = f"FORCED SUSPICIOUS due to Semantic Inference Violation ({ban}). Original: {eval_result.verdict.confidence_reasoning}"
                        break
        ...
        if eval_result.verdict.classification.upper() == "MALICIOUS":
            ...
            if agent_system:
                ...
                try:
                    ...
                    hallucinated = False
                    if not def_result.citations and def_result.verdict.upper() == "OVERRULED_BENIGN":
                        hallucinated = True
                        ...
                        for cit in def_result.citations:
                            if cit and cit != "NONE" and cit not in raw_artifact_str:
                                hallucinated = True
                                break
                        # Extract quoted strings
                        quotes = re.findall(r"'(.*?)'", def_result.benign_explanation) + re.findall(r'"(.*?)"', def_result.benign_explanation)
                        for q in quotes:
                            if q and q not in raw_artifact_str:
                                hallucinated = True
                                ExecutionLogger.log("ORCHESTRATOR", f"DEFENSE ATTORNEY CITATION TRAP: Hallucinated quote '{q}'", "ERROR")
                                break
                    if hallucinated:
                        ExecutionLogger.log("ORCHESTRATOR", "DEFENSE ATTORNEY CITATION TRAP ACTIVATED: Hallucinated technical facts. UPHELD_MALICIOUS.", "ERROR")
                        def_result.verdict = "UPHELD_MALICIOUS"
                    ...
                    if def_result.verdict.upper() == "OVERRULED_BENIGN":
                        ...
                        ExecutionLogger.log("ORCHESTRATOR", "Defense Attorney UPHELD the conviction.", "WARN")
                except Exception as e:
                    ExecutionLogger.log("ORCHESTRATOR", f"Defense Attorney failed: {e}. Upholding Prosecutor.", "ERROR")
...
def main():
    ...
    print(f"\033[96m[ PROJECT MANTIS - V0.3.7 ({mode_str}) ]\033[0m")
    print("------------------------------------------------------------")
