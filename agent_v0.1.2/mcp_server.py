...
def query_json_cache(cache_name: str, keyword: str = "") -> str:
    ...
    try:
        # --- NEW: NATIVE MALFIND PARSER ---
        if cache_name == "malfind" and keyword == "PAGE_EXECUTE_READWRITE":
            try:
                with open(filepath, "r") as f:
                    # Depending on volatility format, this might be a list of lists or a list of dicts.
                    # We will use grep natively but extract ONLY the PIDs.
                    cmd = f"grep -B 5 -i 'PAGE_EXECUTE_READWRITE' {filepath} | grep -i 'PID' | grep -oP '\\d+'"
                    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
                    
                    # Deduplicate the PIDs
                 ...
                 pass # Fallback to default logic if this specific parse fails
        # ----------------------------------
