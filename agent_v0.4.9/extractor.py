...
def run_plugin(image_path, plugin, args=[]):
    ...
    try:
        ...
        ExecutionLogger.log("EXTRACTOR", f"Timeout in {plugin} after 3600s", "ERROR")
...
# Compile regex at the module level for speed
# Captures the Start Sector (Group 1) of standard data partitions
PARTITION_RE = re.compile(
    r"\d{3}:\s+\S+\s+(\d+)\s+\d+\s+\d+\s+(?:NTFS|Linux|Basic data|exFAT|Win95|DOS)", 
    re.IGNORECASE
)

def resolve_disk_offset(evidence_path: str) -> str:
    """
    Dynamically probes the dataset. 
    Returns the string offset if it's a full disk, or None if it's a raw partition.
    """
    try:
        # Probe the image with mmls
        img_type = "ewf" if evidence_path.lower().endswith(".e01") else "raw"
        probe = subprocess.run(["mmls", "-i", img_type, evidence_path], capture_output=True, text=True)
        
        if probe.returncode != 0:
            # Probe failed. The volume system is missing. 
            # Conclusion: It is a raw partition. No offset required.
            return None
            
        # Probe succeeded. Conclusion: It is a full disk.
        # Extract the start sector of the primary file system.
        match = PARTITION_RE.search(probe.stdout)
        if match:
            return match.group(1)
            
    except FileNotFoundError:
        # mmls binary not found in PATH
        pass
...
def generate_bodyfile(disk_image_path, output_path):
    ...
    cmd = ["fls", "-i", img_type, "-r", "-m", "/"]
    if offset:
        cmd.extend(["-o", offset])
    cmd.append(disk_image_path)
        
    start = time.time()
    try:
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            ExecutionLogger.log("EXTRACTOR", f"TSK Command Failed: {result.stderr}", "ERROR")
            return None
...
def extract_lnk_stream(disk_image_path):
    ExecutionLogger.log("EXTRACTOR", f"[DEEP MODE] Streaming LNK file parsing from {disk_image_path}...")
    base_name = os.path.splitext(os.path.basename(disk_image_path))[0]
    safe_base = re.sub(r'[^A-Za-z0-9_\-]', '_', base_name)
    bodyfile_path = os.path.join(CACHE_DIR, f"bodyfile_{safe_base}.txt")
    ...
    lnk_keywords = [":\\\\", "\\\\\\\\", ".exe", ".doc", ".pdf", ".txt", ".zip", "usb", "removable", "volume", "secret"]
    lnk_hits = {}
    
    with open(bodyfile_path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            if "/recent/" in line.lower() and ".lnk" in line.lower().split('|')[1]:
                parts = line.split('|')
                if len(parts) > 3:
                    inode = parts[2].split('-')[0]
                    name = parts[1].split('/')[-1]
                    hits = carve_and_stream_strings(disk_image_path, inode, name, lnk_keywords)
                    if hits:
                        lnk_hits[name] = hits

    if lnk_hits:
        cache_path = os.path.join(CACHE_DIR, f"lnk_stream_{safe_base}.json")
        with open(cache_path, "w") as f:
            json.dump(lnk_hits, f, indent=4)
        ExecutionLogger.log("EXTRACTOR", "LNK streaming to cache complete.", "SUCCESS")

def extract_pcap_stream(pcap_path):
    ExecutionLogger.log("EXTRACTOR", f"[DEEP MODE] Streaming PCAP strings from {pcap_path}...")
    base_name = os.path.splitext(os.path.basename(pcap_path))[0]
    safe_base = re.sub(r'[^A-Za-z0-9_\-]', '_', base_name)
    
    keywords = ["http://", "https://", "get /", "post /", "user-agent:", "host:", "dns", ".com", ".net", ".org", "password", "login", "admin"]
    hits = []
    ...
    try:
        ...
        ExecutionLogger.log("EXTRACTOR", f"PCAP streaming failed: {e}", "WARN")
        return
        
    unique_hits = list(set(hits))
    if unique_hits:
        cache_path = os.path.join(CACHE_DIR, f"pcap_stream_{safe_base}.json")
        with open(cache_path, "w") as f:
            json.dump({os.path.basename(pcap_path): unique_hits}, f, indent=4)
        ExecutionLogger.log("EXTRACTOR", f"PCAP streaming complete. Found {len(unique_hits)} interesting strings.", "SUCCESS")

def main():
    parser = argparse.ArgumentParser(description="Mantis Extractor")
    parser.add_argument("--deep", action="store_true", help="Enable all deep forensics extraction")
    parser.add_argument("--memory", action="store_true", help="Extract only memory artifacts")
    parser.add_argument("--disk", action="store_true", help="Extract disk metadata (bodyfile)")
    parser.add_argument("--registry", action="store_true", help="Extract registry artifacts")
    parser.add_argument("--evtx", action="store_true", help="Extract EVTX streams")
    parser.add_argument("--prefetch", action="store_true", help="Extract Prefetch streams")
    parser.add_argument("--lnk", action="store_true", help="Extract LNK streams")
    parser.add_argument("--pcap", action="store_true", help="Extract PCAP streams")
    args = parser.parse_args()

    ExecutionLogger.log("EXTRACTOR", "Initializing Forensic Extractor Component...")
    
    run_all = not any([args.memory, args.disk, args.registry, args.evtx, args.prefetch, args.lnk, args.pcap])
    
    if run_all or args.memory:
        for agg_file in ["pstree.json", "cmdline.json", "malfind.json", "netscan.json", "hivelist.json"]:
            agg_path = os.path.join(CACHE_DIR, agg_file)
            if os.path.exists(agg_path): os.remove(agg_path)
            
    if run_all or args.registry:
        for agg_file in ["registry_map.json"]:
            agg_path = os.path.join(CACHE_DIR, agg_file)
            if os.path.exists(agg_path): os.remove(agg_path)
    ...
    for f in os.listdir(EVIDENCE_DIR):
        ...
        if img_type == "memory": mem_images.append(file_path)
        elif img_type == "disk": disk_images.append(file_path)
        elif img_type == "network": net_images.append(file_path)
    ...
    for mem_image in mem_images:
        if run_all or args.memory:
            ...
            extract_hivelist(mem_image)
        
    for disk_image in disk_images:
        if run_all or args.disk:
            base_name = os.path.splitext(os.path.basename(disk_image))[0]
            safe_base = re.sub(r'[^A-Za-z0-9_\-]', '_', base_name)
            bodyfile_path = os.path.join(CACHE_DIR, f"bodyfile_{safe_base}.txt")
        ...
        if run_all or args.deep or args.evtx: extract_evtx_stream(disk_image)
        if run_all or args.deep or args.prefetch: extract_prefetch_stream(disk_image)
        if run_all or args.deep or args.lnk: extract_lnk_stream(disk_image)
            
    for net_image in net_images:
        if run_all or args.deep or args.pcap:
            extract_pcap_stream(net_image)
