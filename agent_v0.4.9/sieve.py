...
# Compile the exact patterns (executes in fast C-layer)
TRAMPOLINE_RE = re.compile(r"movabs rax, 0x[0-9a-fA-F]+.*jmp rax", re.IGNORECASE | re.DOTALL)
JIT_RE = re.compile(r"jmp 0x[0-9a-fA-F]+.*add byte ptr.*int3", re.IGNORECASE | re.DOTALL)
CFG_RE = re.compile(r"movabs rax, 0x[0-9a-fA-F]+(?!.*jmp)", re.IGNORECASE | re.DOTALL)

def evaluate_rwx_telemetry(malfind_hits):
    """Evaluates raw instruction arrays and applies deterministic tagging."""
    raw_instructions = " ".join([str(m.get("Disasm", "")) for m in malfind_hits])
    
    if TRAMPOLINE_RE.search(raw_instructions):
        return "SIG_RWX_INJECTION"
    elif JIT_RE.search(raw_instructions) or CFG_RE.search(raw_instructions):
        return "BENIGN_JIT_FLOW"
...
def score_pid_table(pid_table: Dict[int, Any], parent_map: Dict[int, int]) -> List[Tuple[int, int, List[str]]]:
    ...
    for pid, data in pid_table.items():
        ...
        # 1. RWX Injection (evaluation moved to main routing loop)
        malfind_hits = data["malfind"]
        # The multi-dataset routing loop will handle the regex evaluation and dropping
...
def get_suspect_entities(api_budget: int = 30) -> List[Dict]:
                    ...
                    with open(bodyfile_path, "r", encoding="utf-8", errors="ignore") as bf:
                        for line in bf:
                            ...
                            if len(parts) >= 7:
                                ...
                                is_removable_media = "rm_2" in disk_image_name.lower() or "rm_3" in disk_image_name.lower() or "rocba" in disk_image_name.lower()
                                if is_removable_media:
                                    if clean_path.endswith((".docx", ".doc", ".pdf", ".xls", ".xlsx", ".lnk", ".zip", ".rar", ".7z", ".tar", ".gz", ".amr", ".jpg", ".avi", ".png", ".db", ".gif", ".one", ".txt")):
                                        is_leakage = True
                                elif "/users/" in clean_path and ("/desktop/" in clean_path or "/documents/" in clean_path or "/downloads/" in clean_path or "/appdata/" in clean_path):
                                ...
                                if is_malware or is_leakage:
                                    ...
                                    signals_list = ["SIG_DATA_LEAKAGE_INDICATOR"] if is_leakage else ["SIG_ANOMALOUS_DISK_FILE"]
                                    if is_removable_media:
                                        signals_list.append("SIG_REMOVABLE_MEDIA_FILE")
                                    evidence_dict = {
                                    ...
                                    if clean_path.endswith(".zip") or clean_path.endswith(".rar"):
                                        ...
                                        if real_disk_path:
                                            ...
                                            try:
                                                ...
                                                offset = resolve_disk_offset(real_disk_path)
                                    ...
                                    for entry in val:
                                        ...
                                        if hive_type == "SYSTEM":
                                            ...
                                            if real_disk_path:
                                                ...
                                                icat_cmd.extend([real_disk_path, str(entry.get('inode')).split('-')[0]])
                                                subprocess.run(icat_cmd, stdout=open(temp_hive, 'wb'))
    ...
    # ==========================================
    # MULTI-DATASET ROUTING (The Surgical Correction)
    # ==========================================
    routed_entities = []
    for entity in entities:
        
        # ==========================================
        # 1. THE CFREDS ROUTE (Disk & Exfiltration)
        # ==========================================
        if entity.get("type") in ["file", "shortcut", "archive", "prefetch_artifact", "network_capture"]:
            # Let your existing DATA_EXFIL logic handle this.
            # DO NOT apply regex here.
            routed_entities.append(entity) # (Your existing LNK and Staging logic remains untouched below)

        # ==========================================
        # 2. THE ROCBA ROUTE (Memory Analysis)
        # ==========================================
        elif "evidence" in entity and "malfind" in entity.get("evidence", {}):
            malfind_hits = entity["evidence"]["malfind"]
            if malfind_hits:
                raw_instructions = " ".join([str(m.get("Disasm", "")) for m in malfind_hits])
                
                if TRAMPOLINE_RE.search(raw_instructions):
                    entity['score'] = entity.get('score', 0) + 100
                    entity['evidence'].setdefault('signals', []).append("SIG_RWX_INJECTION")
                    if "SIG_RWX_INJECTION" not in entity.setdefault('signals', []):
                        entity['signals'].append("SIG_RWX_INJECTION")
                    routed_entities.append(entity)
                    
                elif JIT_RE.search(raw_instructions) or CFG_RE.search(raw_instructions):
                    # MATHEMATICALLY PROVEN BENIGN JIT.
                    # Safely drop the PROCESS entity without breaking FILE entity loops.
                    entity['status'] = 'BENIGN_JIT_FLOW'
                    continue 
                else:
                    routed_entities.append(entity)
            else:
