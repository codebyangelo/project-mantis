...
def get_suspect_entities(api_budget: int = 30) -> List[Dict]:
    ...
    if pid_table:
        ...
        for pid, score, signals in scored_pids:
            ...
            if score > 0 and (img.endswith(".ex") or len(img) in (14, 15)):
                if args and img.replace(".ex", "").lower() in args.lower():
                    ...
                    with open(bodyfile_path, "r", encoding="utf-8", errors="ignore") as bf:
                        for line in bf:
                            ...
                            if len(parts) >= 7:
                                ...
                                if "/users/" in clean_path and ("/desktop/" in clean_path or "/documents/" in clean_path or "/downloads/" in clean_path):
                                    if clean_path.endswith((".docx", ".doc", ".pdf", ".xls", ".xlsx", ".lnk", ".zip", ".rar")):
                                        # Flag if deleted, or if it matches suspicious keywords
                                        if "(deleted)" in p_lower or any(kw in clean_path for kw in ["resign", "secret", "drive", "dropbox", "usb", "confidential"]):
                                            is_leakage = True
